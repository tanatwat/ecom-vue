	...
	<transition name="fade">
      ...
      <div class="column is-12">
        ...
        <div class="categories">
          <div class="category">
            <li :key="category.id" v-for="category in categories" @click.prevent="selectCategory(category)" :class="{'active': category.id == selected.c }">{{ category.name }}</li>
          </div>
          <div class="category" v-show="selected.c && subcategories.length">
            <li :key="subcategory.id" v-for="subcategory in subcategories" @click.prevent="selectSubcategory(subcategory)" :class="{'active': subcategory.id == selected.sub }">{{ subcategory.name }}</li>
          </div>
          <div class="category" v-show="selected.sub && types.length">
            <li :key="type.id" v-for="type in types" @click.prevent="selectType(type)" :class="{'active': type.id == selected.type }">{{ type.name }}</li>
          </div>
        </div>
      </div>
        ...
        <div v-show="includeDiscount">
          <label class="is-size-5 checkbox" :class="{'active' : selected.discount}">เฉพาะสินค้าลดราคา&nbsp;<input class="" type="checkbox" name="discount" v-model="selected.discount"></label>
        </div>
			</div>
