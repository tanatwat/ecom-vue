<template>
  <div>
    <div class="page-title">
      <h1 class="page-title-heading">Title</h1><a href="#" class="button secondary w-button">ย้อนกลับ</a></div>
    <div class="w-row">
      <div class="left-column w-col w-col-6">
        <div class="box">
          <h3 class="section-heading">ตู้แสดงสินค้าของร้าน</h3>
          <div class="data-list-wrapper w-clearfix">
            <div>
              <div class="data-list-heading">Showcase</div>
            </div>
            <div class="data-list-body">
              <div class="data-list-text">How many products</div>
            </div>
            <div class="action-wrapper"><a href="#" class="button-icon edit w-button">Edit</a><a href="#" class="button-icon delete w-button">Delete</a></div>
          </div>
        </div>
      </div>
      <div class="right-column w-col w-col-6">
        <div class="box">
          <h3 class="section-heading">เพิ่มตู้แสดงสินค้า</h3>
          <div class="w-form">
            <form id="email-form" name="email-form" data-name="Email Form">
              <div class="form-group"><label for="name-2" class="form-label">Label</label><input type="text" id="name-2" name="name-2" data-name="Name 2" maxlength="256" class="form-input w-input"></div>
            </form>
            <div class="w-form-done">
              <div>Thank you! Your submission has been received!</div>
            </div>
            <div class="w-form-fail">
              <div>Oops! Something went wrong while submitting the form.</div>
            </div>
          </div>
          <div class="text-right"><a href="#" class="button submit w-button">ยืนยัน</a></div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
}
</script>

<style lang="css">
</style>
